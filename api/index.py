...
@app.route("/")
def credit():
    return "(CricHD-API) Made With ðŸ’— By apurboBd"

@app.route("/api/<string:channel_id>.m3u8")
def handle_api(channel_id):
    
    source_code = requests.get(f"https://pipcast.cc/embed2.php?v={channel_id}osncric&v=m&q=SuperSport%20Cricket").text 
    print(source_code)
    regex = r"source:\s*['\"](.*?)['\"]"
    print(regex)
...
def handle_api2():
    ...
    response = requests.get(f"https://crichdplayer.xyz/embed2.php?id=osncric&v=m&q=SuperSport%20Cricket{channel_id}", headers={"Referer": "https://crichdplayer.xyz/"})
